...
document.addEventListener('DOMContentLoaded', () => {
    // Fetching all the data first into the server
    fetch('http://localhost:3000/ramens')
    .then((res) => res.json)
    .then(fetchData)

    // define the fetchData function
    function fetchData(foodArray) {
        foodArray.forEach(getRamenElement())
    }

    // displaying images to the DOM and appending them to the Div
    function getRamenElement(menuObj) {
        const img = document.createElement('img')
        const ramenMenu = document.querySelector('#ramen-menu')
        img.src = menuObj.image
        ramenMenu.appendChild(img)

        // give the image powers to be clicked
        img.addEventListener('click', () => {
            // when clicked display all the food's content
            ...
            ramenImg.src = menuObj['image']
            ramenImg.alt = menuObj['name']
            ramenName.textContent = menuObj['name']
            restaurant.textContent = menuObj['restaurant']
            rating.textContent = menuObj['rating']
            comment.textContent = menuObj['comment']
